...
<body>
	...
	<hr/>
	<h3>\${s != s1 && s2 == s3} : ${s != s1 && s2 == s3}</h3> <!-- true -->
	<h3>\${s != s1 and s2 == s3} : ${s != s1 and s2 == s3}</h3> <!-- true -->
	<h3>\${s != s1 || s2 == s3} : ${s != s1 || s2 == s3}</h3> <!-- true -->
	<h3>\${s ne s1 or s2 eq s3} : ${s ne s1 or s2 eq s3}</h3> <!-- true -->
	...
	%>
	<h3>\${empty list} : ${empty list}</h3>
	<!-- list 객체가 생성되어 있고 내부에 요소가 비어있지 않으면 -->
	<h3>\${!empty list} : ${!empty list}</h3>
