...
public class MemberServiceImpl implements MemberService {
	...
	public void withDraw(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (member != null && member.getPass().equals(tempPass)) {
				// db에서 회원 탈퇴 정보 처리
				dao.withDrawMember(member.getNum()); // session에 등록된 회원의 회원번호를 이용해 탈퇴
				// session에서 member 삭제 및 쿠키 정보 삭제
				logOut(request, response);
				pw.println("alert('회원 탈퇴 요청 처리 완료');");
				pw.println("location.href='index.jsp';");
			} else {
				pw.println("alert('회원 탈퇴 요청 처리 실패');");
				pw.println("history.back();");
			}
			pw.println("</script>");
		} catch (IOException e) {
	...
	public void findPassSubmit(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			mail.append("<form action='http://10.100.205.80:8080");
			...
			e.printStackTrace();
			out.println("<script>");
			out.println("alert('서비스에 문제가 있습니다. 다시 이용해 주세요.\\n"+e.getMessage()+"')");
