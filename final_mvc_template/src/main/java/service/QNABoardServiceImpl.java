...
public class QNABoardServiceImpl implements QNABoardService {
	...
	public void boardReplySubmit(HttpServletRequest request) {
		...
		dao.boardReplySubmit(new BoardVO(qna_name, qna_title, qna_content, qna_re_ref, qna_re_lev, qna_re_seq, qna_writer_num));
	}
	...
	public void boardDelete(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (isSuccess) {
				// 삭제 완료 시 게시물 리스트 페이지 이동
				response.sendRedirect("boardList.qna");
			} else {
				// 게시물 게시자 정보와 일치하지 않아 게시물 삭제 실패 시 해당 게시물 상세 보기 페이지로 이동
				response.sendRedirect("qnaAbout.qna?qna_num="+qna_num);
			}
		} catch (Exception e) {
			e.printStackTrace();
