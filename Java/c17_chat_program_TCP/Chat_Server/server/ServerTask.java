...
public class ServerTask implements Runnable{
	...
	// Client에게 메시지를 전달할 출력 스트림
	PrintWriter pw;
	
	// Client에게 메시지를 입력받을 입력 스트림
	BufferedReader br;
	...
	/*
	 * Server 로부터 객체 생성 시 accept된 client socket 정보를 생성자의 매개변수로 전달 받아 필드 초기화 진행
	 */
	...
	// Client에서 출력된 내용을 입력받는 receive 기능 수행
	@Override
	public void run() {
		
		/*
		 *	`/w userID message` -> 귓속말
		 *	`/quit` -> 종료
		 *	나머지는 전체 메시지
		 */
		while(true) {
			try {
				// serverTask에 등록된 클라이언트가 전달하는 메시지를 입력받음
	...
	// message를 전달 받아 등록된 모든 client에게 메시지 전달
	public void broadCast(String message) {
		// hashtable에 저장된 value들의 묶음을 순회 하면서 등록된 모든 client에게 메시지 출력
		for(PrintWriter p : ChatServer.ht.values()) {
			// 메시지를 입력한 사용자는 제외하고 출력
			if(this.pw != p) p.println(message);
		}
	}
	
	// 특정 사용자에게만 메시지 전달(귓속말)
	//	`/w userID message` -> 등록된 사용자 중 Key값이 아이디와 일치하는 사용자에게만 출력
