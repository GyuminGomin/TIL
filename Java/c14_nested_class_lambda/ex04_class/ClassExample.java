package Java.c14_nested_class_lambda.ex04_class;
...
public class ClassExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// newInstance로 생성된 객체의 method또는 생성자에서 예외가 발생 했을때
			// 그 오류를 전달 - 생성자 내부에서 NullPointerException 등
			e.printStackTrace();
		} catch (NoSuchMethodException e) {
			// 동일한 개수의 매개변수를 가진 생성자를 찾지 못했을때
			e.printStackTrace();
		} catch (SecurityException e) {
			// 접근 권한이 없을때 - 파일이 접근할 수 없는 디렉토리에 존재할때
			e.printStackTrace();
		}
