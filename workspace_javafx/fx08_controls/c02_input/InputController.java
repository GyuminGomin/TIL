package workspace_javafx.fx08_controls.c02_input;
	...
	public void initialize(URL location, ResourceBundle bundle) {
		...
		*/
		...
		btnReg.setOnAction((e) -> {
			...
			// ComboBox<String> generic으로 저장된 타입으로 선택된 값을 반환
		...
		btnCancel.setOnAction(e-> {
			...
			comboBox.getSelectionModel().clearSelection(); // 기존 선택한 것이 있으면 clear해주겠다.
			comboBox.setPromptText("선택하세요!");
			
			colorPicker.setValue(new Color(1,1,1,1));
			datePicker.setValue(null); // 선택항목 삭제
			datePicker.setValue(LocalDate.now()); // 현재날짜로 지정
		...
		txtTitle.setOnKeyPressed(new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent event) {
				...
				// keyCode keyEvent를 통해 사용자가 입력한 코드 정보를 저장하는 객체
				if (event.getCode() == KeyCode.ENTER) {
//					txtPass.requestFocus(); // 포커스를 패스워드 필드로 이동
					btnCancel.fire(); // event 강제 실행
				}
			}
		}); // end setOnKeyPressed
