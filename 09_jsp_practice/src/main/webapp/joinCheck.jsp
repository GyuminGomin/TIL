...
<%
	...
	// 사용자가 입력창에 입력한 파라미터들 중 name 값이 id인 값을 읽어옴.
	String id = request.getParameter("id");
	
	// 등록된 회원들 중 동일한 id를 사용하고 있는 사용자가 존재하는지 확인 (중복 아이디 체크)
	MemberVO member = new MemberVO(id);
	
	// 회원목록에 등록된 회원 정보가 하나라도 존재하면(memberList가 비어있지 않으면)
	if (!memberList.isEmpty() && memberList.contains(member)) { // List는 생성되어 있는데 값이 없는 경우가 isEmpty // 매개변수에 있는 변수가 리스트에 존재하는지 확인하는 게 contains
...
<%
	...
	System.out.println("=====================================================");
%>
<!-- 정상적으로 회원 정보가 등록이 완료되었으면, 로그인 페이지로 이동 -->
<script>
	alert('회원가입 성공');
