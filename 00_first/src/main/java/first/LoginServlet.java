...
public class LoginServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String str = req.getQueryString(); // 쿼리 스트링, url 주소표시창에 있는 ? 뒤에 있는 정보를 가져오는 메서드
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		*/ // 한번 읽어들이면, 다시 가져올 수 없으므로 주석 처리
		...
		PrintWriter out = resp.getWriter(); // 2바이트의 문자기반 스트림
		...
		if(id.equals(uid) && pw.equals(upw)) {
			...
			out.println("history.back();"); // 브라우저 페이지 이동 히스토리
			out.println("</script>");
		}
		out.println("</body>");
		out.println("</html>");
